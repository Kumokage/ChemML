...
def _(GridSearchCV, RandomForestRegressor, draw_importance):
    def learn_random_forest(X, y, name):
        parameters = {'n_estimators': range(100, 500, 100), 'max_depth': [None] + list(range(3, 11, 2))}
        model = RandomForestRegressor(random_state=42)
        clf = GridSearchCV(model, parameters, cv=5, scoring='neg_mean_absolute_error', refit=True, n_jobs=-1)
        clf.fit(X, y)
...
def _(GradientBoostingRegressor, GridSearchCV, draw_importance):
    def learn_gradient_boosting(X, y, name):
        parameters = {'learning_rate': [0.5, 0.25, 0.1, 0.05, 0.01], 'n_estimators': [4, 8, 16, 32, 64, 128], 'max_depth': range(1, 18, 2)}
...
def _(GridSearchCV, draw_importance, xgb):
    def learn_xgboost(X, y, name):
        parameters = {'min_child_weight': [1, 5, 7, 10], 'gamma': [0.5, 1, 1.5, 2, 2.5], 'subsample': [0.6, 0.8, 1.0], 'colsample_bytree': [0.6, 0.8, 1.0], 'max_depth': [3, 4, 5]}
...
def _(days_grad, days_rf, days_xgboost, explain):
    explain(days_xgboost, name="XGBoost, days of stability", color_bar=True, is_save=True)
    explain(days_grad, name="GradientBoosting, days of stability", color_bar=True, is_save=True)
    explain(days_rf, name="RandomForest, days of stability", color_bar=True, is_save=True)
    return
...
def _(artifacts_path, days_grad, days_rf, days_xgboost, explain, plt):
    ...
    _y = 0.02
    _x = 1.03
    _txt = AnchoredText('a', loc='upper left', pad=0.25, borderpad=-0.5, prop=dict(fontsize='xx-large'))
    plt.gca().add_artist(_txt)
    explain(days_rf, name='RandomForest, days of stability', color_bar=True)
    plt.tick_params(axis='both', labelsize=16)
    plt.subplot(3, 1, 2)
    _txt = AnchoredText('b', loc='upper left', pad=0.25, borderpad=-0.5, prop=dict(fontsize='xx-large'))
    plt.gca().add_artist(_txt)
    explain(days_grad, name='GradientBoosting, days of stability', color_bar=True)
    plt.tick_params(axis='both', labelsize=16)
    plt.subplot(3, 1, 3)
    _txt = AnchoredText('c', loc='upper left', pad=0.25, borderpad=-0.5, prop=dict(fontsize='xx-large'))
    plt.gca().add_artist(_txt)
